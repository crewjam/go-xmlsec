...
`
	actualSignedString, err := Sign(testSuite.Key, testSuite.DocStr, DsigOptions{})
	c.Assert(err, IsNil)
	c.Assert(string(actualSignedString), Equals, expectedSignedString)

	err = Verify(testSuite.Cert, actualSignedString, DsigOptions{})
	c.Assert(err, IsNil)
}
...
func (testSuite *XmlDSigTest) TestConstructFromSignature(c *C) {
	...
	c.Assert(err, IsNil)
	actualSignedString, err := Sign(testSuite.Key, docStr, DsigOptions{})
	c.Assert(err, IsNil)
...
func (testSuite *XmlDSigTest) TestVerifyFailsWhenMessageModified(c *C) {
	...
	signedStr, err := Sign(testSuite.Key, testSuite.DocStr, DsigOptions{})
	c.Assert(err, IsNil)

	err = Verify(testSuite.Cert, signedStr, DsigOptions{})
	c.Assert(err, IsNil)

	signedStr = []byte(strings.Replace(string(signedStr), "Hello", "Goodbye", 1))
	err = Verify(testSuite.Cert, []byte(signedStr), DsigOptions{})
	c.Assert(err, Equals, ErrVerificationFailed)
}

func (testSuite *XmlDSigTest) TestInvalidXML(c *C) {
	_, err := Sign(testSuite.Key, []byte("<invalid xml"), DsigOptions{})
	c.Assert(err, ErrorMatches, "malformed XML")

	_, err = Sign(testSuite.Key, []byte("<invalid></invalid>"), DsigOptions{})
	c.Assert(err, ErrorMatches, "cannot find start node")

	_, err = Sign([]byte("XXX"), testSuite.DocStr, DsigOptions{})
	c.Assert(err, ErrorMatches, "failed to load pem key")

	err = Verify(testSuite.Cert, []byte("<invalid xml"), DsigOptions{})
	c.Assert(err, ErrorMatches, "malformed XML")

	err = Verify(testSuite.Cert, []byte("<invalid></invalid>"), DsigOptions{})
	c.Assert(err, ErrorMatches, "cannot find start node")

	err = Verify([]byte("XXX"), testSuite.DocStr, DsigOptions{})
	c.Assert(err, ErrorMatches, "xmlSecCryptoAppKeyLoadMemory failed")

	err = Verify(testSuite.Key, testSuite.DocStr, DsigOptions{})
	c.Assert(err, ErrorMatches, "xmlSecCryptoAppKeyLoadMemory failed")

	err = Verify(testSuite.Cert, testSuite.DocStr, DsigOptions{})
...
-----END CERTIFICATE-----`)
	...
	err := Verify(cert, doc, DsigOptions{
		XMLID: []XMLIDOption{{
