...
`
	actualSignedString, err := Sign(testSuite.Key, testSuite.DocStr, Options{})
	c.Assert(err, IsNil)
	c.Assert(string(actualSignedString), Equals, expectedSignedString)

	err = Verify(testSuite.Cert, actualSignedString, Options{})
	c.Assert(err, IsNil)
}
...
func (testSuite *XmlDSigTest) TestConstructFromSignature(c *C) {
	...
	c.Assert(err, IsNil)
	actualSignedString, err := Sign(testSuite.Key, docStr, Options{})
	c.Assert(err, IsNil)
...
func (testSuite *XmlDSigTest) TestVerifyFailsWhenMessageModified(c *C) {
	...
	signedStr, err := Sign(testSuite.Key, testSuite.DocStr, Options{})
	c.Assert(err, IsNil)

	err = Verify(testSuite.Cert, signedStr, Options{})
	c.Assert(err, IsNil)

	signedStr = []byte(strings.Replace(string(signedStr), "Hello", "Goodbye", 1))
	err = Verify(testSuite.Cert, []byte(signedStr), Options{})
	c.Assert(err, Equals, ErrVerificationFailed)
}

func (testSuite *XmlDSigTest) TestInvalidXML(c *C) {
	_, err := Sign(testSuite.Key, []byte("<invalid xml"), Options{})
	c.Assert(err, ErrorMatches, "malformed XML")

	_, err = Sign(testSuite.Key, []byte("<invalid></invalid>"), Options{})
	c.Assert(err, ErrorMatches, "cannot find start node")

	_, err = Sign([]byte("XXX"), testSuite.DocStr, Options{})
	c.Assert(err, ErrorMatches, "failed to load pem key")

	err = Verify(testSuite.Cert, []byte("<invalid xml"), Options{})
	c.Assert(err, ErrorMatches, "malformed XML")

	err = Verify(testSuite.Cert, []byte("<invalid></invalid>"), Options{})
	c.Assert(err, ErrorMatches, "cannot find start node")

	err = Verify([]byte("XXX"), testSuite.DocStr, Options{})
	c.Assert(err, ErrorMatches, "xmlSecCryptoAppKeyLoadMemory failed")

	err = Verify(testSuite.Key, testSuite.DocStr, Options{})
	c.Assert(err, ErrorMatches, "xmlSecCryptoAppKeyLoadMemory failed")

	err = Verify(testSuite.Cert, testSuite.DocStr, Options{})
